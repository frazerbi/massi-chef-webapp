...
- **Preventivo raggruppato per categorie (20/08/2026, CL-1)**: le righe del preventivo sono ora raggruppate per categoria nella pagina e nel PDF, con intestazioni e senza subtotali (totale unico in fondo). La logica di raggruppamento/ordinamento è una funzione pura unica, `lib/calc/raggruppamentoPreventivo.ts`, importata sia dalla pagina sia dal PDF (§3: mai duplicarla). Ordine: antipasto, primo, secondo, contorno, dessert, finger, altro, Consumabili apparecchio, Consumabili cucina, Altri prodotti, Servizi e altre voci (righe extra sempre in coda; i valori dell'enum restano `apparecchiatura`/`consumabile`, le etichette mostrate sono state cambiate il 20/08/2026 su richiesta dell'utente); i gruppi vuoti non vengono stampati; le righe ricetta senza portata disponibile finiscono in "Altre portate". Per separare apparecchiatura e consumabili serviva un dato affidabile (`consumabile.categoria` è testo libero): migrazione `0008_tipo_consumabile.sql` con enum `categoria_consumabile_tipo` e colonna `tipo_consumabile` (default `consumabile`), esposta anche nel form dell'anagrafica consumabili. Risolto anche il punto scoperto in analisi: `calcolaPreventivo` ora carica ed espone le ricette (come già faceva per materie prime e consumabili), altrimenti la `categoria_portata` non era disponibile a nessuno dei due punti di presentazione. **Nessun costo ricalcolato**: puro riordino di presentazione, i preventivi inviati mostrano gli stessi importi di prima (invariante 1). Migrazione `0008_tipo_consumabile.sql` **applicata su Supabase** (20/08/2026, via SQL Editor) e anagrafica riclassificata: dei 16 consumabili esistenti, 11 sono `apparecchiatura` (piatti, bicchieri, posate, palette caffè, tovaglioli) e 5 restano `consumabile` (buste e rotoli sotto vuoto, pellicola, spago, vassoi alluminio) — criterio: va in tavola davanti all'ospite → apparecchio, serve a cucina/produzione/trasporto → cucina. Il vecchio campo testo libero `consumabile.categoria` resta popolato ma non ha più alcun effetto sul raggruppamento.
- **Chiusura della coda di fase 1 (20/08/2026)**: cinque voci completate prima di aprire la fase 2. (1) **FEATURE-011** — pagine `[id]` di modifica per consumabili e bevande, con form condiviso creazione/modifica (`components/FormConsumabile.tsx`, `components/FormBevanda.tsx`) sul modello di `FormMateriaPrima`: `aggiornaConsumabile`/`aggiornaBevanda` esistevano in `/lib/db` ma non erano richiamate da nessuna UI. (2) **Fix quota beveraggio** — la limitazione segnalata il 02/08 è risolta: la quota % di un prodotto già assegnato è modificabile inline (`aggiornaQuotaProdottoBeveraggio`, validazione lato server: quota in (0,100] e somma di categoria ≤ 100 escludendo il prodotto in modifica) e a copertura 100% compare la spiegazione invece del modulo di aggiunta nascosto. Nessuna migrazione. (3) **FEATURE-013** — menu raggruppato per portata riusando `raggruppaRighePreventivo` di CL-1; `menu_riga.ordine` resta sotto-ordinamento manuale dentro il gruppo. (4) **FEATURE-009** — colonna `marca` su `materia_prima`, migrazione `0009_materia_prima_marca.sql` **applicata su Supabase** (20/08/2026, via SQL Editor); campo descrittivo, non entra in nessun calcolo. (5) **UX-001/002/003** — il form materia prima è ora client component per mostrare la domanda giusta secondo l'unità scelta ("Quanti grammi in 1 kg?") e l'esempio live della resa ("su 1 kg acquistato, utilizzabili: X g"); testi introduttivi nella sezione beveraggio del preventivo e su `/profili-beveraggio`. Nessuna formula spostata fuori da `/lib/calc/`.
